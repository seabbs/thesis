...
% Everything below added by CII
  ...
  In this chapter I have introduced the \texttt{getTBinR} R package. \texttt{getTBinR} facilitates downloading the most up-to-date version of multiple TB relevant data sources from the WHO, along with the accompanying data dictionaries. It also contains functions to allow easy exploration of the data via searching data dictionaries, summarising key metrics on a regional and global level, and visualising the data in a variety of highly customisable ways. In addition, it provides both a dashboard and an automated country level report that enables the global, regional, and country level picture to be quickly summarised. It was developed using R package development best practices and has been peer-reviewed.{[}43{]}
  ...
  Whilst \texttt{getTBinR} is feature complete, and stable, development work continues. Future projects include: using the \texttt{shinymeta} R package to provide downloadable R code to users of the interactive application; iterating on the current automated report to improve formatting and to increase the amount of information displayed. As additional WHO TB data are released they will be added to \texttt{getTBinR}.
