...
```{r 07-inc-rate-summary-plot, fig.cap = "Incidence rates (with 95% confidence intervals) per 100,000 per year for UK born population and non-UK born population, aged 0-5 and therefore directly affected by the targeted neonatal vaccination programme, and aged 14-19 and therefore directly affected by the universal school-age scheme."}
...
make_summary_table <- function(df, caption = NULL, caption.short = NULL) {
  ...
  df$`IRR (CI 95%)*` <-  stringr::str_replace_all(df$`IRR (CI 95%)*`, 
                                              "0.00 \\(0.00, 0.00\\)", 
                                              "1.00 \\(1.00, 1.00\\)")
  df %>% 
  add_footnote_ref() %>% 
...
EstMagImpactBestModel %>% 
  dplyr::mutate(`Vaccination Programme` = `Vaccination Programme` %>%
                  str_replace_all("\u2020", "**")) %>% 
  dplyr::select(`Vaccination Programme (age)` = `Vaccination Programme`, everything()) %>% 
