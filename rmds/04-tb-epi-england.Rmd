...
### Enhanced tuberculosis surveillance (ETS) system {#ets-deep-dive}
...
created_var <- tibble(
  ...
  Description = c('Derived using year of vaccination and year of notification. Categorised into $\\leq 10$ and 11+ due to the evidence of waning protection for the BCG vaccine.[@Abubakar2013]', 
                  'Derived using year of vaccination and age at vaccination. Categorised into < 1, 1  to 11, 12 to 16 and $16+$ to capture historic vaccination policy.[@PublicHealthEngland2011a]', 
                  ...
                  'Assessed via follow up at 12 and 24 months: mortality is defined as cases with an overall outcome of death, and survival is defined as those that completed treatment, were still on treatment, or stopped treatment. Those that were lost to follow up, or not evaluated were treated as missing', 
...
pander::pander(created_var, caption = '(\\#tab:created-var) Variables derived or modified from the ETS system for use in the analyses throughout this thesis.', booktabs = TRUE, justify = 'left', split.cells = 50)
```
...
```{r data-struct, fig.scap = "Summary plot of the structure of the ETS data extract used in this thesis.", fig.cap = "Summary plot of the structure of the ETS data extract used in this thesis. Due to the large size of the dataset, the data has been sub sampled with only 20\\% of the data shown. Notifications have been ordered by date of notification from left to right. Purple indicates numeric variables, green indicates factor variables, dates are shown using red and samples with missing values are shown using grey. The figure shows that the majority of the included variables are factors, with a significant minority of numeric and date variables. Data completeness is near random for some variables but has changed other time for others, for example national socioeconomic quintile (natquintile). This figure does not account for nested variables", out.width = "60%", eval = FALSE, out.extra = ""}
if (regen_results) {
...
kable(miss_ets,
      caption.short = "Breakdown of missing data from the ETS prior to the web based system (pre 2009) and post (post 2008) by variable, ordered by the  percentage missing for a subset of variables.",
...
```{r bcgvacc-miss}
model_miss_results[["bcgvacc"]] %>% 
  dplyr::select(-`P value (Wald)`) %>% 
  rlang::set_names(., nm = names(.) %>% stringr::str_replace_all("\\(LRT\\)", "")) %>% 
kable(
      caption.short = "Results from a logistic regression model with data completeness (Complete/Missing) for BCG vaccination as an outcome, adjusted for: year, sex, age (grouped as 0-14 year olds, 15-65 year olds and 65+), ethnic group, UK birth status and socio-economic status (national quintiles).",
...
```{r bcgvaccyr-miss}
model_miss_results[["bvcvaccyr"]] %>% 
  dplyr::select(-`P value (Wald)`) %>% 
  rlang::set_names(., nm = names(.) %>% stringr::str_replace_all("\\(LRT\\)", "")) %>% 
kable(format = "latex",
      caption.short = "Results from a logistic regression model with data completeness (Complete/Missing) for year of BCG vaccination as an outcome, adjusted for: year, sex, age (grouped as 0-14 year olds, 15-65 year olds and 65+), ethnic group, UK birth status and socio-economic status (national quintiles).",
...
```{r dateofdeath-miss}
model_miss_results[["dateofdeath"]] %>% 
  dplyr::select(-`P value (Wald)`) %>% 
  rlang::set_names(., nm = names(.) %>% stringr::str_replace_all("\\(LRT\\)", "")) %>% 
kable(
      caption.short = "Results from a logistic regression model with data completeness (Complete/Missing) for date of death as an outcome, adjusted for: year, sex, age (grouped as 0-14 year olds, 15-65 year olds and 65+), ethnic group, UK birth status and socio-economic status (national quintiles).",
...
model_miss_results[["tomdeathrealat"]] %>% 
  dplyr::select(-`P value (Wald)`) %>% 
  rlang::set_names(., nm = names(.) %>% stringr::str_replace_all("\\(LRT\\)", "")) %>% 
kable(
      caption.short = "Results from a logistic regression model with data completeness (Complete/Missing) for cause of death as an outcome, adjusted for: year, sex, age (grouped as 0-14 year olds, 15-65 year olds and 65+), ethnic group, UK birth status and socio-economic status (national quintiles).",
...
```{r symptonset-miss}
model_miss_results[["symptonset"]] %>% 
  dplyr::select(-`P value (Wald)`) %>% 
  rlang::set_names(., nm = names(.) %>% stringr::str_replace_all("\\(LRT\\)", "")) %>% 
kable(
      caption.short = "Results from a logistic regression model with data completeness (Complete/Missing) for date of symptom onset as an outcome, adjusted for: year, sex, age (grouped as 0-14 year olds, 15-65 year olds and 65+), ethnic group, UK birth status and socio-economic status (national quintiles).",
...
```{r datediag-miss}
model_miss_results[["datediag"]] %>% 
  dplyr::select(-`P value (Wald)`) %>% 
  rlang::set_names(., nm = names(.) %>% stringr::str_replace_all("\\(LRT\\)", "")) %>% 
kable(
      caption.short = "Results from a logistic regression model with data completeness (Complete/Missing) for date of diagnosis onset as an outcome, adjusted for: year, sex, age (grouped as 0-14 year olds, 15-65 year olds and 65+), ethnic group, UK birth status and socio-economic status (national quintiles).", 
...
model_miss_results[["starttreatdate"]] %>% 
  dplyr::select(-`P value (Wald)`) %>% 
  rlang::set_names(., nm = names(.) %>% stringr::str_replace_all("\\(LRT\\)", "")) %>% 
kable(
      caption.short = "Results from a logistic regression model with data completeness (Complete/Missing) for date of starting treatment as an outcome, adjusted for: year, sex, age (grouped as 0-14 year olds, 15-65 year olds and 65+), ethnic group, UK birth status and socio-economic status (national quintiles).", 
...
```{r txenddate-miss}
model_miss_results[["txenddate"]] %>% 
  dplyr::select(-`P value (Wald)`) %>% 
  rlang::set_names(., nm = names(.) %>% stringr::str_replace_all("\\(LRT\\)", "")) %>% 
kable(
      caption.short = "Results from a logistic regression model with data completeness (Complete/Missing) for date of starting treatment as an outcome, adjusted for: year, sex, age (grouped as 0-14 year olds, 15-65 year olds and 65+), ethnic group, UK birth status and socio-economic status (national quintiles).",
      caption = "Results from a logistic regression model with data completeness (Complete/Missing) for date of ending treatment as an outcome, adjusted for: year, sex, age (grouped as 0-14 year olds, 15-65 year olds and 65+), ethnic group, UK birth status and socio-economic status (national quintiles). For socio-economic group 1 indicates the most deprived quintile. Notifications from 2010 onwards were included as socio-economic status was not collected before this. Complete case analysis was used. Odds ratios shown are adjusted for all explanatory variables.", booktabs = TRUE, longtable = TRUE) %>% 
  kable_styling(font_size = 8, latex_options = c("repeat_header")) %>% 
  column_spec(c(1), width = "1.5cm") %>% 
  column_spec(c(4), width = "2cm") %>% 
  column_spec(c(6), width = "1.5cm") %>% 
...
```{r plot-overall-pop, fig.scap = "Overall population estimates in England derived using ONS and LFS demographic data.", fig.cap = "Overall population estimates in England derived using ONS (Total) and LFS (Total (LFS)) demographic data. The ONS data is likely to be more reliable as the LFS data is derived using a weighted survey. After accounting for missing UK birth status both datasets provide comparable estimates of the population of England, with a clearly increasing trend over time. However, the ONS data indicates a reduction in population from 2000 until 2001 that is not seen in the LFS data. The UK born and non-UK born populations are estimated using the LFS data.", out.extra = ""}
knitr::include_graphics("chapters/tb-epi-england/figures/plot-overall-pop.png")
```

```{r plot-age-strat, fig.scap = "Percentage difference between ONS population estimates and estimates derived from the LFS by 5 year age group.", fig.cap = "Percentage difference between ONS population estimates and estimates derived from the LFS by 5 year age group. For most age groups there is less than a 2\\% difference over time. In older adults (85+) there is a substantially greater difference ranging from 5\\% to 40\\%.", out.extra = ""}
...
```{r age-dist-pop, fig.scap = "The estimate proprotion of the population in each 5 year age group stratified by UK birth status for 2000, 2008, and 2016.", fig.cap = "The estimate proprotion of the population in each 5 year age group stratified by UK birth status for 2000, 2008, and 2016.", out.extra = ""}
knitr::include_graphics("chapters/tb-epi-england/figures/age-dist-pop.png")
```

## TB incidence rates {#tb-inc-rates-epi}

### Motivation

As discussed in Section \@ref(age-dist-nots) and Section \@ref(pop-dist-england), changes in underlying population demographics may mask or bias trends in TB notifications. To account for this, incidence rates, which indicate the incidence of TB for a standard population size, may be used. Whilst TB incidence rates are available in the yearly PHE TB report,[@PHE2017] they are limited in detail and do not report age stratified, or UK born stratified incidence rates across years. Estimating these incidence rates will allow for novel analyses to be conducted later in this thesis that explore population adjusted trends in TB. The method used to estimate incidence rates is first outlined, then overall trends in incidence rates, stratified by UK birth status are explored. Finally trends in incidence rates, stratified by age and UK birth status, are investigated.
...
Age-standardised rates were calculated using the `r 'epiR'` package for R,[@Stevenson2016] using the average age distribution of England from 2000-2015 as the standard population to allow comparison between years. Those aged 90+ were excluded as demographic data for this population were unreliable. The code used to calculate incidence rates is available online as an R package (`tbinenglanddataclean`^[`tbinenglanddataclean`: https://github.com/seabbs/tbinenglanddataclean]; see Chapter \@ref(introduction)).
...
- The code used in this chapter to import, clean and manipulate the data sources has been made accessible separately as an R package (`tbinenglanddataclean`; see Chapter \@ref(introduction)), along with documentation of the required data sources and package functions. If interested in reproducing this work from the raw data please see this documentation for details.
