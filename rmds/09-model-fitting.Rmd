...
kable(
  ...
  caption = "Observed versus predicted overall TB incidence for years that the model was fitted too. The model underpredicted incidence for all years fitted too. The upper credible interval of the model predictions was substantially lower than the observed data.", booktabs = TRUE, longtable = TRUE, escape = TRUE) %>% 
  kable_styling(latex_options = c("hold_position")) %>% 
  add_footnote(label = "* 95% credible interval estimated using the 2.5% quantile and the 97.5% quantile.") %>% 
  {.}
```
...
```{r 09-fig-prior-params, fig.scap = "Prior and posterior distributions for fitted model parameters.", fig.cap = "Prior and posterior distributions for fitted model parameters. Parameter names in figure are in their coded form. They can be interpreted using the following: beta_young_adult - $\\beta_{\\text{young adult}}$"}
knitr::include_graphics(file.path(plots_path, "prior-posterior-overview-1.png"))
```


```{r 09-prior-posterior, warning = FALSE}
sum_prior_posterior %>% 
  set_names(., nm = . %>% 
...
kable(
  ...
  kable_styling(latex_options = c("hold_position")) %>% 
  add_footnote(label = c("* 95% credible interval estimated using the 2.5% quantile and the 97.5% quantile."), notation = "none") %>% 
  {.}
```
...
```{r 09-fig-sens, fig.scap = "Partial rank correlation coefficients for each parameter fitted too.", fig.cap = "Partial rank correlation coefficients for each parameter fitted too. Parameter names in figure are in their coded form. They can be interpreted using the following: beta_young_adult - $\\beta_{\\text{young adult}}$, c_eff - $c_{\\text{eff}}$, c_hist - $c^{\\text{hist}}_{\\text{eff}}$, c_hist_half - $c^{\\text{hist}}_{\\text{half}}$, M - $M$, M_young_adult - $M_{\\text{young adult}}$, meas_error - $E_{\\text{syst}}$, meas_std - $E_{\\text{noise}}$, non_uk_scaling - $\\iota_{\\text{scale}}$, older_adult_activation_scaling - $\\epsilon^{\\text{older-adult}}_L$"}
