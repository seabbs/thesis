...
2. 2000 parameter sets were taken from the prior distribution and the model was then simulated for each one. 
...
7. SMC-SMC model fitting with 2000 initial parameter particles. Particle rejuvernation was set to trigger when the effective sample size decreased below 25%, with 10 MCMC steps used each time. 
...
After development of the model fitting pipeline but before results could be produced both the `optimise` (from `rbi`) and `adapt_particle` (from `rbi.helpers`) began to error with a difficult to debug `LibBi` error message. This meant that steps 3-5 of the model fitting pipeline could not be used with the final model. As a work around the maximum permitted number of particles (256) was then used for all model fitting (increasing the number of particles was also explored as detailed in the previous section). 
...
```{r 09-scenarios-table, eval = FALSE}
tibble(Scenario = names(scenario_dics) %>% str_split("_") %>% 
  ...
  str_to_sentence()) %>% 
kable(
  caption = "DIC values for each scenario evaluated during model fitting - arranged from best to worst model fit.", booktabs = TRUE, longtable = TRUE) %>% 
...
```{r 09-table-incidence-preds, eval = FALSE}
sum_obs_table %>% 
  set_names(., nm = c(names(.)[-ncol(.)], "Predicted Incidence (95% CI)*")) %>% 
kable(
  caption.short = "Observerd versus predicted overall TB incidence for years that the model was fitted too.", 
  caption = "Observerd versus predicted overall TB incidence for years that the model was fitted too. The model underpredicted incidence for all years fitted too. The upper credible interval of the model predictions was substantially lower than the observed data. ", booktabs = TRUE, longtable = TRUE) %>% 
  kable_styling(latex_options = c("hold_position")) %>% 
  add_footnote(label = "* 95% credible interval estimated using the 2.5% quantile and the 97.5% quantile.", notation = "none")
```
...
```{r 09-format-variables-names}
params <- sum_prior_posterior$Parameter %>% 
  replace_code_with_form()

legend_params <- map2_chr(sum_prior_posterior$Parameter, 
                          params,
                          ~ paste(.x, .y,  sep = " - ")) %>% 
  paste0(collapse = ", ")
```

```{r 09-fig-prior-params, fig.scap = "Prior and posterior distributions for fitted model parameters.", fig.cap = paste0("Prior and posterior distributions for fitted model parameters. Parameter names in figure are in their coded form. They can be interpreted using the following: ", legend_params)}
knitr::include_graphics(file.path(plots_path, "prior-posterior-overview-1.png"))
```


```{r 09-prior-posterior}
sum_prior_posterior %>% 
  set_names(., nm = . %>% 
...
kable(
  caption.short = "Observerd versus predicted overall TB incidence for years that the model was fitted too.", 
  caption = "Observerd versus predicted overall TB incidence for years that the model was fitted too. The model underpredicted incidence for all years fitted too. The upper credible interval of the model predictions was substantially lower than the observed data. ",
  booktabs = TRUE, longtable = TRUE, escape = TRUE) %>% 
  kable_styling(latex_options = c("hold_position")) %>% 
  add_footnote(label = c("* 95% credible interval estimated using the 2.5% quantile and the 97.5% quantile."),
                        notation = "none")
```
