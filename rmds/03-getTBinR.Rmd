...
Whilst `getTBinR` is feature complete, and stable, development work continues. Future projects include: using the `shinymeta` R package to provide downloadable R code to users of the interactive application; iterating on the current automated report to improve formating and to increase the amount of information displayed. As additional WHO TB data are released they will be added to `getTBinR`.
